...
# TS: -1, the root is not accurate (The finding should be the same as NR). See my revision

secant.method <- function(f, x0, x1, tol = 1e-100, n = 10000) {
  # calculate the new x value at the first iteration
  x2 <- x1 - f(x1) / ((f(x1) - f(x0)) / (x1 - x0))
  # start the 2nd iteration
  for (i in 2:n) {
    if (abs(x2 - x1) < tol) { # If the difference between the 
      #new value and the previous value is small enough, end iteration and output root.
      x_root <- x2
